...
像您与人类交谈时一样要求Claude用西班牙语回复。就是这么简单！"""
...
给Claude一个可能使Claude更擅长解决数学问题的角色！"""

# 练习4.1的提示
exercise_4_1_hint = """本练习中的评分函数正在寻找包含"haiku"和"pig"单词的解决方案。
不要忘记在您希望主题被替换的地方包含确切的短语"{TOPIC}"。更改"TOPIC"变量值应该使Claude写一首关于不同主题的俳句。"""

# 练习4.2的提示
exercise_4_2_hint = """本练习中的评分函数正在寻找包含"brown"单词的响应。
如果您用XML标签包围"{QUESTION}"，这如何改变Claude的响应？"""

# 练习4.3的提示
exercise_4_3_hint = """本练习中的评分函数正在寻找包含"brown"单词的响应。
尝试一次删除一个单词或一段字符，从最不合理的部分开始。一次做一个单词也将帮助您看到Claude能够或不能解析和理解多少内容。"""

# 练习5.1的提示
exercise_5_1_hint = """本练习的评分函数正在寻找包含"Warrior"单词的响应。
用Claude的声音写更多单词来引导Claude按您希望的方式行动。例如，您可以写"Stephen Curry is the best and here are three reasons why. 1:"，而不是"Stephen Curry is the best because,"。"""

# 练习5.2的提示
exercise_5_2_hint = """评分函数寻找超过5行长度且包含"cat"和"<haiku>"单词的响应。
从简单开始。目前，提示要求Claude写一首俳句。您可以改变并要求两首（甚至更多）。然后如果您遇到格式化问题，在您已经让Claude写了多于一首俳句之后，更改您的提示来修复问题。"""

# 练习5.3的提示
exercise_5_3_hint = """本练习中的评分函数正在寻找包含"tail"、"cat"和"<haiku>"单词的响应。
将这个练习分解为几个步骤是有帮助的。								
1.	修改初始提示模板，使Claude写两首诗。							
2.	给Claude指示器说明诗歌将是关于什么的，但不要直接写入主题（例如，狗、猫等），而是用关键词"{ANIMAL1}"和"{ANIMAL2}"替换这些主题。							
...
让我们逐步完成这个练习：										
1.	Claude如何知道您要使用什么类别？告诉它！直接在提示中包含您想要的四个类别。确保也包含括号字母以便于分类。随意使用XML标签来组织您的提示，并向Claude明确类别从哪里开始和结束。									
2.	尝试减少多余的文本，使Claude立即用分类回答，并且只用分类回答。有几种方法可以做到这一点，从为Claude说话（提供从句子开头到单个开放括号的任何内容，以便Claude知道您希望括号字母作为答案的第一部分）到告诉Claude您想要分类，只要分类，跳过前言。
如果您想复习这些技术，请参考第2章和第5章。							
3.	Claude可能仍然错误分类或在回答时不包含类别名称。通过告诉Claude在其答案中包含完整类别名称来修复这个问题。								
4.	确保您的提示模板中仍然有{email}，以便我们可以正确地为Claude评估替换电子邮件。"""
...
有时最简单的方法是给Claude一个您希望其输出看起来像什么的示例。只是不要忘记将您的示例包装在<example></example>标签中！并且不要忘记，如果您预填Claude的响应，Claude实际上不会将其作为响应的一部分输出。"""

# 练习7.1的提示
exercise_7_1_hint = """您需要编写一些示例电子邮件并为Claude分类（使用您想要的确切格式）。有多种方法可以做到这一点。以下是一些指导原则。										
1.	尝试至少有两个示例电子邮件。Claude不需要所有类别的示例，示例不必很长。为您认为较困难的类别提供示例更有帮助（您在第6章练习1底部被要求考虑的）。XML标签将帮助您将示例与提示的其余部分分开，尽管这不是必需的。									
2.	确保您的示例答案格式完全是您希望Claude使用的格式，以便Claude也可以模仿该格式。此格式应该使Claude的答案以类别字母结尾。无论您将{email}占位符放在哪里，确保它的格式完全像您的示例电子邮件。									
3.	确保您在提示本身中仍然列出了类别，否则Claude不会知道要引用什么类别，以及{email}作为替换的占位符。"""
...
如果Claude不知道答案，它应该做什么？"""

# 练习8.2的提示
exercise_8_2_hint = """本练习中的评分函数正在寻找包含短语"49-fold"的响应。
通过首先提取相关引用并查看引用是否提供足够证据，让Claude显示其工作和思考过程。如果您想复习，请参考第8章课程。"""
